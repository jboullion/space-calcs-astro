...
function importFirebaseData() { // Import the data from firebase and finish startup

	if (typeof firebase === 'undefined') { // Firebase not available
	...
	} else { // Firebase Available

		// Only start generating after ten seconds
		setTimeout(function () {
			if (!orbitalPositions["earth"]) { // If Firebase takes too long, just generate it anyway

				// Log the course of action
				console.log("No firebase return, Generating now: " + round((new Date().getTime() - now.getTime()) / 1000, 2) + "s")
		...
		firebase.database().ref('/').once('value').then(function (snapshot) {
			...
			console.log("Firebase Return: " + round((new Date().getTime() - now.getTime()) / 1000, 2) + "s")
			...
			if (!orbitalPositions["earth"]) {
				...
				for (var name in tempParams) {
					...
					for (var key in tempParams[name]) {
						if (tempParams[name][key] != originalPlanets[name][key] && key != "mapClass" && key != "viewingClass" && key != "axialTilt") {
							same = false;
						}
					}

					// Check if parameters the same
					if (!same || tempTimes[name].length != orbitResolution * 360) { // If they are different, generate from first principles

						// Log if there is a detected discrepancy
						if (!same) {
							console.log("WARNING: Parameter discontinuity detected at " + capitalise(name))
						}
						else if (tempTimes[name].length != orbitResolution * 360) {
							console.log("WARNING: Resolution discontinuity detected at " + capitalise(name))
						}
					...
					else { // If they are the same, just use the Firebase ones

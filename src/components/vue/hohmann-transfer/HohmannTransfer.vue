			...
			<div class="calc-form mb-5 p-2 rounded border">
				...
				<table class="table">
					<tbody>
						<tr
							:style="{
								backgroundColor: getOrbitColorHex(
									formData.startOrbit,
								),
							}"
						>
...
const startingOrbitVelocity = computed(() => {
	return (
		...
		(formData.value.startOrbit.orbitalPeriod *
			scaleConversions.secondsToDays)
	);
});
...
const aphelionVelocity = computed(() => {
	return (
		((2 * Math.PI * semiMajorAxis.value) / transferPeriod.value) *
		Math.sqrt(
			(2 * semiMajorAxis.value) / formData.value.endOrbit.distance - 1,
		)
	);
});
...
function updateRenderSize() {
	...
	threeCanvas.style.paddingTop = `0px`;
}

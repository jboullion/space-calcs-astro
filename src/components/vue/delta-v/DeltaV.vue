<template>
  <div id="delta-v__app" class="row mt-5" v-if="formData" v-cloak>
    ...
    <div id="delta-v__results" class="col-lg-8 calc-form">
      ...
      <!-- <GChart type="LineChart" @ready="drawDeltaVChart" /> -->

      <div class="result-chart mt-4 mb-4">
        <div id="delta-v-chart" style="width: 100%; height: 400px"></div>
      </div>
      <!-- 
      <div class="result-chart mt-4 mb-4">
        <div id="fuel-chart" style="width: 100%; height: 400px"></div>
      </div>

      <div class="result-chart mb-5" v-show="formData.showC3">
        <div id="c3-chart" style="width: 100%; height: 400px"></div>
      </div>

      <button
        type="button"
        class="btn btn-nexus"
        @click="formData.showC3 = !formData.showC3"
      >
        {{ formData.showC3 ? "Hide" : "Show" }} C3
      </button>

      <button
        type="button"
        class="btn btn-nexus"
        @click="formData.showMap = !formData.showMap"
      >
        {{ formData.showMap ? "Hide" : "Show" }} Map
      </button> -->
    </div>
  </div>
</template>
